...
public class BookSellerAgent extends Agent {
  ...
  protected void setup() {
	...
	pending = new Hashtable();//initialize
  ...
  public void updateCatalogue(final String title, final int price) {
	...
	private class OfferRequestsServer extends CyclicBehaviour {
	  public void action() {
	    ...
	    if (msg != null) {
	      ...
	      //Integer price = (Integer) catalogue.get(title); exercise5
	...
	private class PurchaseOrdersServer extends CyclicBehaviour {
	  public void action() {
	    ...
	    if (msg != null) {
	      ...
	      ACLMessage reply = msg.createReply();
	     // Integer price = (Integer) catalogue.remove(title);
	      Integer price = (Integer) pending.remove(title);
		  if (price != null) {
	...
	private class PurchaseOrderRejection extends CyclicBehaviour {
		...
		public void action() {
			...
			//exercise5
			if (msg != null) {
				Integer price = (Integer) pending.remove(msg.getContent());
				System.out.println(getAID().getLocalName() + ": Proposal has been rejected!");
				if (price != null) catalogue.put(msg.getContent(), price);
			} else {
				block();
			}
		}
