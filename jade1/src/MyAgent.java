...
class MyCyclicBehaviour extends CyclicBehaviour {
	MyAgent myAgent;
	int id=0;
	HashMap<String, String> wordId = new HashMap<String, String>();
	...
	public void action() {
		...
		if (message == null) {
			...
			if (performative == ACLMessage.REQUEST)
			{
				...
				try
				{
					...
					else
					{
						...
						id=id+1;
						wordId.put(String.valueOf(id),content);
						forward.setReplyWith(String.valueOf(id));
			...
			{	
				//task4
				String recivedId = message.getInReplyTo();
				
				//when it is an answer
				myAgent.displayHtmlResponse(content+wordId.get(recivedId));
			}
		}
	}
}
